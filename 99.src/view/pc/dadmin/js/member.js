...
function init_pagination() {
    ...
    for (let i = page_start; i <= page_end; i++) {
        ...
        if (i == page_cur) {
            ...
            item = '<li><a href="./member.php?page=' + i + '" class="pg_link">' + i + '</a></li>';
        }
...
function movePage(page) {
    if (page > 0) {
        location.href = './member.php?page=' + page;
    }
}
...
function outputExcel(data) {
    if (data.length > 0) {
        JwExcel.exportExcel(data, '한돈 소믈리에 회원 정보');
    } else {
...
function doSearch() {
    ...
    location.href = './member.php?' + getObjQueryString(paramObj)
}